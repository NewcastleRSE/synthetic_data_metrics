...
from utils import is_categorical, prep_data_updated
...
def _calculate_ds(real, synth, window_size, step, epochs,
                 verbose):
    ...
    X, y = prep_data_updated(data.drop('label', axis=1), data.label, window_size=window_size, step=step)
    # shuffle the two lists
    c = list(zip(X, y))
    random.shuffle(c)
    X, y = zip(*c)
    X = np.asarray(X, dtype=np.float32) # .reshape(-1, seq_length, input_dim)
    y = np.asarray(y)
...
def dis_score(real, synth, target: str = None, window_size=10,
    ...
    Returns:
        score: A single score or the average discriminative scores of all
        labels.
    """
    ...
    if target is not None:
        ...
        for label in labels:
            ...
            ds_temp = _calculate_ds(real_temp, synth_temp, window_size,
                                   step, epochs, verbose)
            disc_scores.append(ds_temp)
    # else if the dataset has no target column pass it as a whole
    else:
        result = _calculate_ds(real, synth, window_size, step, epochs,
                              verbose)
